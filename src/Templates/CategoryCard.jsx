...
{Category.map((elem)=>(
    <div className="flex flex-col rounded-2xl cursor-pointer border-2 border-[#f2f3f5] bg-white" key={elem.id} onClick={() => setActiveCategory(elem)}  >
    <div className="flex h-[280px] rounded-t-2xl "  style={{
    ...
    <div className="flex gap-2">
                <div className="rounded-full w-4 h-4 mt-1.5 "
style={{ backgroundColor: elem.color }} > 
</div>
<div className="flex flex-col">
<p className="text-[18px] font-extrabold">{elem.title}</p>
<span className="text-[#6b7280] text-[12px]">Lorem ipsum dolor, sit amet consectetur adipisicing elit.</span>
</div>
    </div>
    <button className="py-1 px-3 h-[30px] rounded-full border-[#e4e5e8] border text-[11px] " style={{color:elem.color , backgroundColor: hexToRgba(elem.color, 0.08) }}>see all</button>
    </div>

</div>
))}
</div>
<CategoryModal category={activeCategory}
